...
| `standard` | *(défaut)* + protection des garde-fous, garde des migrations, refus des fichiers de travail (une fois par chemin), refus des serveurs de dev au premier plan, commit-gate sur le contenu indexé, avertissements d'hygiène Bash, fact-forcing sur commandes destructives, gate qualité et rappel des compagnons sur `Stop`, compression des sorties Bash |
...
| Événement | Contrôle |
|---|---|
| `PreToolUse` Edit/Write | refuse un secret écrit en clair (`secret-guard`) ; refuse une migration goose avec `;` en commentaire ou sans `Down`, avertit sur `DROP` sans `IF EXISTS` et AND/OR non parenthésés (`migration-guard`) ; refuse une fois par chemin et par session la création d'un fichier de travail (`NOTES.md`, `report.md`, `rapport-final.md`, `todo.txt`…) hors de `docs/`, `.claude/`, `.github/` et du vault — la seconde tentative passe (`no-artifact-files`) ; refuse d'éditer une config de lint/format/types ; fact-forcing (strict) |
...
2. **Aucun gate ne boucle.** Le fact-forcing ne se déclenche qu'une fois par
   ...
   revient jamais deux fois pour la même règle dans une session. Le refus des fichiers de travail suit la
   même règle (une fois par chemin). Le commit-gate, lui, se répète tant que le
